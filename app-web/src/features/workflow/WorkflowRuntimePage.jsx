...
import { ChatMessageRow } from '../../panels/ChatMessageRow.jsx';
import { ChatTimelineChevron } from '../../panels/ChatTimelineNodes.jsx';
import { ScrollToBottomButton } from '../../panels/ScrollToBottomButton.jsx';
import { SettingsLucideIcon } from '../settings/settings-ui.jsx';
import {
  WorkflowFlowNode,
  workflowEdgeAppearance,
} from '../settings/WorkflowConfigEditor.jsx';
    ...
    <aside className="workflow-detail-panel" aria-label={`${node.label || node.id} execution details`}>
      ...
      <header className="workflow-detail-head">
        <span className="workflow-detail-icon" aria-hidden="true">
          <SettingsLucideIcon name={NODE_ICON[node.type] || 'box'} size={20} />
        </span>
        ...
        <div ref={detailBodyRef} className="workflow-detail-body chat-message-list" aria-live="polite">
          {status === 'waiting_input' ? (
            <div className="workflow-detail-waiting" role="status">
              <SettingsLucideIcon name="message" size={18} />
  ...
  if (!workflow?.nodes?.length) {
    return (
      <div className="workflow-run-empty">
        <SettingsLucideIcon name="git-branch" size={28} />
