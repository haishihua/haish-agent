...
test('embedded rendering drops the approval card header and flattens into the tool card', () => {
  ...
  assert.ok(overlaySource.includes('.chat-timeline-tool > .haish-approval-slot.is-embedded'));
  assert.ok(overlaySource.includes('.haish-approval-card.is-embedded .haish-approval-body'));
  // The first-row warning icon is gone: no ::before on the embedded intent.
  assert.ok(!overlaySource.includes('.haish-approval-intent::before'));
});
