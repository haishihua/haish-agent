...
| [ask-user-card.html](fixtures/ask-user-card.html) | 生产 AskUserInlineForm + 生产 approvalStore：占位提示写明「可加备注，也可自己写答案」，题目翻页是 26px 描边圆按钮 + `Question 1 of 2` + 悬停说明且真的能切题，最后一题/第一题的禁用边界、提交门禁与「选选项 + 备注 / 纯手写」的答案形状逐项断言；`window.__askUserChecks()` 返回逐项断言 |
