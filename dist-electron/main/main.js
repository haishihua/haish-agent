...
async function proxyApiRequest(request, url) {
    ...
    try {
        ...
        return Response.json({
            detail: `Local runtime is unavailable: ${String(error?.message || error)}`,
        }, { status: 503 });
    }
...
function getWindowVisualState(window) {
    return {
        fullScreen: window.isFullScreen(),
        maximized: window.isMaximized(),
    };
}
function publishWindowVisualState(window) {
...
async function resolveInsideProject(projectId, relativePath = '') {
...
ipcMain.handle('skill:pick-directory', async () => {
    const result = await dialog.showOpenDialog({
...
ipcMain.handle('fs:list-directory', async (_event, projectId, relativePath = '') => {
