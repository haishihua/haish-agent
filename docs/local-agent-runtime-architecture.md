...
>
> 目标本地 runtime：`/Users/zhanruitao/py-project/haish-agent-core`
>
> 设计结论：Haish 桌面端应从“云端 agent runtime + 本地 workspace bridge”迁移为“客户端托管本地 agent runtime + 云端模型网关/同步服务”。第一阶段主要改造 `haish-agent`，`haish-agent-core` 复用现有本地 workspace/tool/app 能力，只做 sidecar 启动、配置和少量 API 适配。
...
因此，本方案不是重写 `haish-agent-core`，而是恢复它原本更擅长的本地 runtime 形态：客户端启动一个本机 sidecar，UI 直接请求本机 runtime，本机 runtime 直接使用真实 workspace tools。云端只保留模型网关、账号、配置、计费和可选同步。

## 1.1 设计修订

经过进一步讨论，本方案按以下判断收敛：

- `haish-agent-core` 原有 `ReadFileTool(workspace)`、`ListDirTool(workspace)`、`SearchTextTool(workspace)`、`TerminalTool(workspace)` 等逻辑本来就是本地 workspace 模型，不需要大改。
...
- `haish-agent-core` 作为本地 agent runtime，复用已有 app/agent/tool 能力，负责会话、agent loop、工具、模型调用、事件流。
...
- 不重写 `haish-agent-core` 的 agent 框架，也不重写已有 workspace tool 逻辑。
...
第一阶段不应把 runtime 侧改成全新架构。只有当现有 `haish-agent-world-web` 无法满足 sidecar 进程管理时，才新增薄入口。
...
```bash
python -m haish_agent_core.app.local_runtime \
  --host 127.0.0.1 \
  --port 0 \
...
```json
{
  "message": "/Users/zhanruitao/py-project/haish-agent-core 你来分析一下这个项目",
  "options": {
...
{"type":"llm_tool_call_requested","tool_name":"list_dir","arguments":{"path":"/Users/zhanruitao/py-project/haish-agent-core"}}
...
|---|---|---|
| `py-project/haish-agent-core` | `/Users/zhanruitao` | 允许 |
| `/Users/zhanruitao/py-project/haish-agent-core` | `/Users/zhanruitao` | 允许 |
| `/Users/zhanruitao/haish-agent-core` | `/Users/zhanruitao` | 路径不存在 |
...
- 客户端启动本地 `haish-agent-core` runtime。
...
- 提问 `/Users/zhanruitao/py-project/haish-agent-core 你来分析一下这个项目`。
...
### 13.2 haish-agent-core

第一阶段轻量适配仓库。优先复用现有 `haish-agent-world-web` / app API；只有缺口明确时才新增：

```text
src/haish_agent_core/app/local_runtime.py      # 可选：sidecar 薄入口
src/haish_agent_core/app/local_settings.py     # 可选：本地 data/log/model gateway 配置
src/haish_agent_core/app/model_gateway_client.py # 可选：如果现有 llm adapter 不能直接配置网关
```

建议调整：

```text
src/haish_agent_core/app/assistant.py
  - 尽量复用 selected workspace prompt
  - 必要时补充 root 内绝对路径说明

src/haish_agent_core/tool/workspace_read.py
src/haish_agent_core/tool/workspace_write.py
src/haish_agent_core/tool/terminal.py
  - 可选：统一路径描述
  - 保持 resolve 后不能越过 workspace root

src/haish_agent_core/app/api.py
  - 优先复用任务流协议
  - 如存在云端部署假设，再做最小抽离
```
...
```text
src/haish_agent_core/app/remote_fs_tools.py
src/haish_agent_core/app/workspace_bridge.py
```
...
2. 提问 `/Users/zhanruitao/py-project/haish-agent-core 你来分析一下这个项目`。
